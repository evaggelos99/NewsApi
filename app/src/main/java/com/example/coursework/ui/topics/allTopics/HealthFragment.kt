...
class HealthFragment : Fragment(), onItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val bool = preferences?.getBoolean("health", false)
        val preferredCountry = preferences?.getString("preferred_country", "gb")
        if (bool!!) {
            val request = Request.Builder()
                .url(BASE_URL + TOP_HEADLINES + "country=" + preferredCountry + "&" + "category=health&" + API_KEY)
                .build()
            ...
            client.newCall(request).enqueue(object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    val recyclerViewLayout =
                        view.findViewById(R.id.health_recView) as RecyclerView

                    activity?.runOnUiThread {
                        val llm = LinearLayoutManager(context)
                        llm.orientation = LinearLayoutManager.VERTICAL
                        recyclerViewLayout.setLayoutManager(llm)
                        recyclerViewLayout.setAdapter(MyAdapter(articles, this@HealthFragment))
    ...
    override fun onItemClick(item: Article, position: Int) {
        ...
        var clicks = preferences?.getInt("healthClicks", 0)
        val editor = preferences?.edit()
        if (clicks != null) {
            editor?.putInt("healthClicks", clicks+1)
            editor?.commit()
        }
