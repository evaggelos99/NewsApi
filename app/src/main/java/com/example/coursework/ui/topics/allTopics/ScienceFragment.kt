...
class ScienceFragment : Fragment(), onItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val bool = preferences?.getBoolean("science", false)
        val preferredCountry = preferences?.getString("preferred_country", "gb")
        if (bool!!) {
            val request = Request.Builder()
                .url(BASE_URL + TOP_HEADLINES + "country=" + preferredCountry + "&" + "category=science&" + API_KEY)
                .build()
            ...
            client.newCall(request).enqueue(object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    val recyclerViewLayout =
                        view.findViewById(R.id.science_recView) as RecyclerView

                    activity?.runOnUiThread {
                        val llm = LinearLayoutManager(context)
                        llm.orientation = LinearLayoutManager.VERTICAL
                        recyclerViewLayout.setLayoutManager(llm)
                        recyclerViewLayout.setAdapter(MyAdapter(articles, this@ScienceFragment))
    ...
    override fun onItemClick(item: Article, position: Int) {
        ...
        var clicks = preferences?.getInt("scienceClicks", 0)
        val editor = preferences?.edit()
        if (clicks != null) {
            editor?.putInt("scienceClicks", clicks+1)
            editor?.commit()
        }
